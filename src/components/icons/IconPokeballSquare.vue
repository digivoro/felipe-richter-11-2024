<template>
  <svg viewBox="0 0 48 48" id="a" xmlns="http://www.w3.org/2000/svg">
    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
    <g
      id="SVGRepo_tracerCarrier"
      stroke-linecap="round"
      stroke-linejoin="round"
    ></g>
    <g
      id="SVGRepo_iconCarrier"
      stroke="currentColor"
      fill="none"
      stroke-linecap="round"
      stroke-linejoin="round"
    >
      <path
        class="d"
        d="m5.5008,35.6714c-.0005-.0572-.0008-.1145-.0008-.1718,0-10.2173,8.2827-18.5,18.5-18.5s18.5,8.2827,18.5,18.5c0,.0573-.0003.1146-.0008.1718"
      ></path>
      <path
        class="d"
        d="m5.5611,37.68c3.3753-3.6377,7.8298-6.2599,12.8544-7.3573"
      ></path>
      <path
        class="d"
        d="m42.5,37.68c-3.3753-3.6377-7.8298-6.2599-12.8544-7.3573"
      ></path>
      <circle class="d" cx="24" cy="29.3665" r="5.2365"></circle>
      <g id="b">
        <path
          id="c"
          class="d"
          d="m40.5,5.5H7.5c-1.1046,0-2,.8954-2,2v33c0,1.1046.8954,2,2,2h33c1.1046,0,2-.8954,2-2V7.5c0-1.1046-.8954-2-2-2Z"
        ></path>
      </g>
      <circle class="d" cx="24" cy="29.3665" r="2.5"></circle>
    </g>
  </svg>
</template>
